        ...
        public async Task<IActionResult> OnGetAsync(int? branchId)
        {
            ...
            if (empIds.Any())
            {
                ...
                foreach (var p in Payslips)
                {
                    ...
                    decimal gross = p.BasicSalary + bonusSum;
                    decimal ded = epfEmp + p.TaxDeduction;
                    decimal net = gross - ded;

                    if (p.Bonuses != bonusSum || p.GrossPay != gross || p.TotalDeductions != ded || p.NetPay != net || p.HousingAllowance != 0 || p.TransportAllowance != 0 || p.MedicalAllowance != 0 || p.EpfEmployee != epfEmp || p.EpfEmployer != epfCo || p.Etf != etf)
                    {
                        p.Bonuses = bonusSum;
