...
namespace HRMS.UI.Pages.Admin.DutyAccounts
{
    ...
    public class CreateDutyAccountModel : PageModel
    {
        ...
        public bool HasExistingWelfareManager { get; set; }
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            if (SelectedRole == "Area Manager" || SelectedRole == "HR Manager" || SelectedRole == "Welfare Manager" || SelectedRole == "Department Head")
            {
            ...
            var welfareDept = await _context.Departments.FirstOrDefaultAsync(d => d.Name == "Welfare")
                              ?? (await _context.Departments.FirstOrDefaultAsync());
            ...
            switch (SelectedRole)
            {
                ...
                {
                    var existingWM = await _userManager.GetUsersInRoleAsync("Welfare Manager");
                    if (existingWM.Any() || await _context.Users.AnyAsync(u => u.UserName == "head.welfare"))
                    {
                        ModelState.AddModelError("", "A Welfare Department Head duty account already exists (head.welfare). Only one Welfare Department Head is permitted.");
                    }
                    break;
                }
            ...
            AccountDisplayName = SelectedRole switch
            {
                ...
                "Welfare Manager"   => "Welfare Department Head",
            ...
            var resolvedDeptId = SelectedRole switch
            {
                ...
                "Welfare Manager"   => (welfareDept?.Id ?? hrDept.Id),
            ...
            var desigEntity = await _context.Designations.FirstOrDefaultAsync(d => d.Title == SelectedRole)
                              ?? await _context.Designations.FirstOrDefaultAsync(d => d.Title == "Department Head")
            ...
            try
            {
                ...
                var user = new ApplicationUser
                {
                    ...
                    Branch             = SelectedRole == "Area Manager"
                                            ? AreaName.Trim()
                                            : SelectedRole == "HR Manager" || SelectedRole == "Welfare Manager"
                                                ? headOffice.Name
                    ...
                    Department         = SelectedRole == "HR Manager"
                                            ? hrDept.Name
                                            : SelectedRole == "Welfare Manager"
                                                ? (welfareDept?.Name ?? "Welfare")
                                                : SelectedRole == "Department Head"
                ...
                if (SelectedRole == "Welfare Manager")
                {
                    if (!await _roleManager.RoleExistsAsync("Department Head"))
                        await _roleManager.CreateAsync(new IdentityRole("Department Head"));
        ...
        private async Task LoadDropdownsAsync()
        {
            ...
            // 0.1 Check Welfare Manager existence
            var welfareManagerUsers = await _userManager.GetUsersInRoleAsync("Welfare Manager");
            HasExistingWelfareManager = welfareManagerUsers.Any() || await _context.Users.AnyAsync(u => u.UserName == "head.welfare");
