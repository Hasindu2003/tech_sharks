...
namespace HRMS.UI.Pages.BranchManager
{
    ...
    public class ReviewResignationModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(int id, string action, string comments)
        {
            ...
            bool success;
            if (action == "approve")
                success = await _resignationService.BranchManagerApproveAsync(id, comments, user.Email!);
            else
                success = await _resignationService.BranchManagerRejectAsync(id, comments, user.Email!);

            TempData[success ? "SuccessMessage" : "ErrorMessage"] = success
                ? (action == "approve" ? "Resignation acknowledged and forwarded to Area Manager." : "Resignation request has been rejected.")
                : "Unable to process this request.";

            return RedirectToPage("/BranchManager/ReviewResignations");
        }
    }
}
