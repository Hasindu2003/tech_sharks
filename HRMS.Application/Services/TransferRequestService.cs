...
namespace HRMS.Application.Services
{
    ...
    public class TransferRequestService : ITransferRequestService
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(userId)) return (string.Empty, string.Empty);

            var user = await _context.Users.FirstOrDefaultAsync(u => u.Id == userId || u.UserName == userId || u.Email == userId);
        ...
        {
            if (request == null || string.IsNullOrWhiteSpace(branch) || string.IsNullOrWhiteSpace(department))
                return false;
        ...
        public async Task<List<TransferRequestViewModel>> GetRequestsForDeptHeadAsync(string branch, string department)
        {
            ...
            return matching.Select(MapToViewModel).ToList();
        }
