...
namespace HRMS.Application.Services
{
    public interface IResignationService
    {
        ...
        Task<(bool Success, string? Error)> ValidateAndSubmitAsync(int id, string? requestedByRole = null);
        ...
        Task<bool> IsManagerialRequestAsync(ResignationRequest request);
    ...
    public class ResignationService : IResignationService
    {
        ...
        public async Task<(bool Success, string? Error)> ValidateAndSubmitAsync(int id, string? requestedByRole = null)
        {
            var entity = await _context.ResignationRequests
                .Include(r => r.Documents)
                .Include(r => r.DepartmentReviews)
            ...
            bool isManager = await IsManagerialEmployeeAsync(entity.EmployeeEmail, entity.EpfNumber, entity.Designation, requestedByRole, entity.Department);
            if (isManager)
            {
                entity.Status = ResignationStatus.PendingHRReview;
                entity.LastModifiedDate = DateTime.Now;
                await _context.SaveChangesAsync();
                ...
                await SendNotificationsAsync(
                    hrRecipients,
                    "Managerial Resignation Request Submitted (For Review)",
                    $"Resignation notice #{entity.Id} for {entity.EmployeeName} ({entity.Designation}, {entity.Branch}) has been submitted for HR review.",
                ...
                await SendNotificationsAsync(
                    ...
                    $"Your resignation request #{entity.Id} has been submitted directly to the HR Manager for review.",
                    CoreNotificationType.Info,
                    entity.Id,
        ...
        public static bool IsManagerialDept(string? name)
        {
            ...
            return norm == "managerial" || norm == "management" || norm.Contains("managerial") || norm.Contains("management");
        }
        ...
        public async Task<List<ResignationRequestViewModel>> GetPendingForDeptHeadAsync(string branchName, string departmentName)
        {
            ...
            var nonManagerial = new List<ResignationRequest>();
            bool anyPromoted = false;
            foreach (var r in pendingForThisDept)
            {
                if (await IsManagerialRequestAsync(r))
                {
                    if (r.Status != ResignationStatus.PendingHRReview && r.Status != ResignationStatus.ManagerReviewed)
                    {
                        r.Status = ResignationStatus.PendingHRReview;
                        r.LastModifiedDate = DateTime.Now;
                        if (r.DepartmentReviews.Any())
                        {
                            _context.ResignationDepartmentReviews.RemoveRange(r.DepartmentReviews);
                            r.DepartmentReviews.Clear();
                        }
                        anyPromoted = true;
                    }
                    continue;
            ...
            return nonManagerial.Select(Map).ToList();
        }
        ...
        public async Task<List<ResignationRequestViewModel>> GetReviewedByDeptHeadAsync(string branchName, string departmentName)
        {
            ...
            var nonManagerialReviewed = new List<ResignationRequest>();
            foreach (var r in reviewedForThisDept)
            {
                if (await IsManagerialRequestAsync(r)) continue;
                nonManagerialReviewed.Add(r);
            }
            return nonManagerialReviewed.Select(Map).ToList();
        }
            ...
            string reviewerUserId)
        {
            ...
            if (await IsManagerialRequestAsync(entity))
                return false;
        ...
        public async Task<List<ResignationRequestViewModel>> GetPendingForHRManagerAsync(List<int>? managedBranchIds = null)
        {
            var query = _context.ResignationRequests
                ...
                .Include(r => r.Documents)
                .Include(r => r.DepartmentReviews)
                .Where(r => r.Status == ResignationStatus.AMApproved || r.Status == ResignationStatus.PendingHRReview);
            ...
            var list = await query.OrderByDescending(r => r.AMReviewDate ?? r.ResignationDate).ToListAsync();
        ...
        public async Task<bool> HRManagerMarkAsReviewedAsync(int id, string comments, string reviewerEmail)
        {
            ...
            if (entity == null || entity.Status != ResignationStatus.PendingHRReview)
                return false;

            entity.HRReview     = "Reviewed";
            entity.HRReviewDate = DateTime.Now;
            entity.HRComments   = comments;
            entity.HREmail      = reviewerEmail;
            entity.Status       = ResignationStatus.ManagerReviewed;
            entity.AcceptanceLetterGenerated = false;
            entity.LastModifiedDate          = DateTime.Now;

            await _context.SaveChangesAsync();

            // 1. Notify Employee / Manager
            var empIds = await GetEmployeeUserIdentifiersAsync(entity.EmployeeEmail, entity.EpfNumber);
            await SendNotificationsAsync(
                empIds,
                "Resignation Notice Acknowledged by HR 👁️",
                $"Your resignation notice #{entity.Id} has been seen and acknowledged by the HR Manager. Further managerial resignation and clearance proceedings will take place outside of this system.",
                CoreNotificationType.Info,
                entity.Id,
                $"/Resignation/Details/{entity.Id}"
            );
        ...
        public async Task<bool> HRManagerApproveAsync(int id, string comments, string reviewerEmail)
        {
            ...
            entity.AcceptanceLetterGenerated = false;
            entity.AcceptanceLetterDate      = null;
            entity.LastModifiedDate          = DateTime.Now;
            ...
            await SendNotificationsAsync(
                empIds,
                "Resignation Approved by HR ✅",
                $"Your resignation has been officially finalized and approved by HR. Last working day: {entity.EffectiveDate:MMMM dd, yyyy}.",
                CoreNotificationType.Approved,
                entity.Id,
                $"/Resignation/Details/{entity.Id}"
            );
        ...
        public async Task<bool> IsManagerialRequestAsync(ResignationRequest request)
        {
            if (request == null) return false;
            if (request.Status == ResignationStatus.PendingHRReview ||
                request.Status == ResignationStatus.ManagerReviewed)
                return true;

            return await IsManagerialEmployeeAsync(request.EmployeeEmail, request.EpfNumber, request.Designation, null, request.Department);
        }
